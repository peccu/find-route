  ...
  <div>
    <form @submit.prevent="onSave" class="space-y-3">
      <div class="flex gap-2">
        <button class="px-3 py-2 bg-blue-600 text-white rounded cursor-pointer">
          保存
        </button>
        <button
          type="button"
          @click="$emit('cancel')"
          class="px-3 py-2 bg-gray-200 rounded cursor-pointer"
        >
          キャンセル
        </button>
      </div>
      ...
      <div class="space-y-2">
        ...
        >
          <div class="flex justify-between items-center mb-2">
            ...
            <div class="flex gap-2">
              <button
                ...
                @click="moveUp(idx)"
                :disabled="idx === 0"
                class="px-2 py-1 bg-gray-200 rounded cursor-pointer disabled:bg-gray-400 disabled:cursor-not-allowed disabled:opacity-50"
              >
                ↑
              </button>
              <button
                type="button"
                @click="moveDown(idx)"
                :disabled="idx === form.legs.length - 1"
                class="px-2 py-1 bg-gray-200 rounded cursor-pointer disabled:bg-gray-400 disabled:cursor-not-allowed disabled:opacity-50"
              >
                ↓
              </button>
              <button
                type="button"
                @click="removeLeg(idx)"
                class="px-2 py-1 bg-red-300 rounded cursor-pointer"
              >
                削除
              </button>
            </div>
          ...
          <div v-if="leg.type === 'walk'">
            ...
            <input
              ...
              inputmode="numeric"
              pattern="[0-9]*"
              v-model.number="(leg as any).durationMinutes"
          ...
          <div v-else>
            ...
            <div class="mt-2">
              ...
              <div class="flex gap-2">
                <input
                  v-model="(leg as any).url"
                  class="border rounded p-1 w-full"
                />
                <button
                  type="button"
                  @click="open(idx)"
                  :disabled="!(leg as any).url"
                  class="px-3 py-2 bg-blue-600 text-white rounded cursor-pointer flex-shrink-0 disabled:bg-gray-400 disabled:cursor-not-allowed disabled:opacity-50"
                >
                  Open
                </button>
              </div>
        ...
        <div class="flex gap-2 mt-2">
          <button
            type="button"
            @click="addWalk"
            class="px-3 py-1 bg-green-400 rounded cursor-pointer"
          >
            徒歩区間を追加
          </button>
          <button
            type="button"
            @click="addTrain"
            class="px-3 py-1 bg-indigo-400 text-white rounded cursor-pointer"
          >
            電車/バス区間を追加
          </button>
        </div>
