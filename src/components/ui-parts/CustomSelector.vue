<template>
  <div class="inline-block">
    ...
    <div class="relative inline-block w-60" ref="dropdownRef">
      ...
      <div
        v-if="isOpen"
        class="absolute top-full left-0 right-0 mt-1 bg-white border rounded shadow-lg z-10"
      >
        <button
          v-for="item in items"
          :key="String(item)"
        ...
        >
          ...
          <svg
            v-if="modelValue && modelValue[valueKey] === item[valueKey]"
...
const selectedItem = computed(
  () =>
    props.items.find(
      (item) =>
        props.modelValue &&
        item[props.valueKey] === props.modelValue[props.valueKey],
    ) || null,
)
